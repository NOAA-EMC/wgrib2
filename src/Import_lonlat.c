 ...
 * By default grib2 saves the latitude and longitude in micro-degrees. 
 * This is more precision than the standard single-precision floating point 
 * variable can hold. So wgrib2 uses double precision variables for its angles. 
 * However, the -rpn facility is single precision, so it cannot be use for angles 
 * without losing precision. The -export_lonlat and -import_lonlat options allow 
 * you to write and read double-precision longitude and latitudes. 
 * 
 * ## File format
 ...
 * 
 * Prior to wgrib2 v3.1.2, -import_lonlat would always read the first record of the input file. 
 * For wgrib2 v3.1.2+, -import_lonlat would read the records sequentially. If there was an EOF, 
 * then read would start at the beginning of the file. Consequently old behavior mimicked for 
 * files with only one record. 
 * 
 * ## Caution
 * The -import_lonlat will change the internal values of the longitudes and latitudes. However, 
 * wgrib2 will still use the original grid organization. For example, the grib message was a 
 * lat-long grid, and replaced the values for a polar stereographic grid. Some parts of wgrib2 may 
 * still assume the grid is a lat-lon grid. The -import_lonlat will be more useful for unstructured 
 * grids like GDT 3.101 (General Unstructured Grid). GDT 3.101 does not include geolocation, and 
 * -import_lonlat can add the geolocation. Consequently the nearest neighbor interpolation will then 
 * work. 
 * 
 * ## Usage
 * -import_lonlat FILE
