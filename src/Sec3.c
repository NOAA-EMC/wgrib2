/** @file
 * @brief Some routines that involve Section 3 (Grid Definition Section).
 * 
 * ### Program History Log
 ...
 * 8/2007 | B. Vuong     | Bug fix - mercator variable dimension had wrong location
...
/** Pointer to variable dimensions. */
int *variable_dim = NULL;

/** Pointer to raw variable dimensions. */
int *raw_variable_dim = NULL;
...
/**
 * Prints a short summary of Section 3 (Grid Definition Section).
 * 
 * ## Usage
 * -Sec3
 * 
 ...
 * @param nx Pointer to the number of grid points in the x direction.
 * @param ny Pointer to the number of grid points in the y direction.
 * @param npnts Pointer to the number of points in the grid.
 * @param res Pointer to the grid resolution (Resolution and Component Flags Table 3.3).
 * @param scan Pointer to the scan mode (Table 3.4).
 * 
 * @return 0 for success, error code otherwise
 * 
 * @author Wesley Ebisuzaki @date 1/2007
...
/**
 * Figures out nx and ny (unsigned int).
 * 
 * @param sec Pointer to GRIB sections.
 * @param nx Pointer to the number of grid points in the x direction.
 * @param ny Pointer to the number of grid points in the y direction.
...
/**
 * Prints the size of a "rectangular' grid. Of course not all grids are rectangular 
 * (ex. thinned Gaussian grids).
 *
 * ## Usage
 * -nxny
 * 
...
/**
 * Prints the scan order of the grid.
 * 
 * A grib message contains the grid point values. If you are a C programmer, the grid would 
 * naturally be in "C" order. A fortran programmer would have the grid in fortran order. Some 
 * people would start from the NW corner and others would start from the SW corner. In grib1, 
 * all these scan orders are possible. The grib1 scan orders are, 
 * 
 * 1. WE:SN
 * 2. WE:NS
 * 3. EW:SN
 ...
 * @endcode
 * 
 * This "plow the field" order reduces the file size when using regional grids and saving the 
 * increments. 
 * 
 * Grib2 has 16 scan orders of which 3 are common: WE:SN, WE:NS and WE|EW:SN. Wgrib2 simplfies 
 * life by internally converting fields to WE:SN by default. 
 * 
 * ## Usage
 * -scan
 * 
...
/** Array of scan orders for JMA grid. */
...
/**
 * Prints the number of grid points for each row of the grid. This is necessary for a thinned 
 * grid where the number of grid points per row decreases as you approach the poles. 
 * 
 * ## Usage
 * -nlons
 * 
...
/**
 * Prints out the grid information.
 * 
 * ## Understanding the grid definitions
 * The grid definitions are based on the grib grib defintions as published by the WMO and copied 
 * by the NCEP's grib documentation. [For example, the lat-lon grid defintion.]
 ...
 * - output WE:SN    .. wgrib2 internally has the data is in WE:SN scan order. If wgrib2 writes a
 * bin/ieee/text file, it will be in the output scan order. The output scan order has to be WE:SN 
 * for geolocation to work. The default output scan order is WE:SN.
 * - output WE:NS    .. wgrib2 internally has the data is in WE:NS scan order. The is enabled by 
 * -order we:ns, and is used for writing binary data in WE:NS order.
 * - output raw      .. wgrib2 internally has the data in the input scan order. This scan order is 
 * needed for -new_grid to work.  
 * - res N           .. value of the resolution and component flags octet (byte)
 * - lat X to Y by Z .. latitudes start at X goes to Y by steps of Z
 * - lon X to Y by Z .. longitudes start at X goes to Y by steps of Z (Note: grib2 uses longitude 
 * is in [0,360) model)
 * - #points N       .. number of points in the grid including grid points with undefined values
 * 
 * ## dlat and dlon in lat-lon grids
 * For calculating the lat/lon of the grid points, wgrib2 calculates a more accurate version of dlat and 
 * dlon from the end points of the grid. For all the angle calculations, wgrib2 uses double precision. 
 ...
 * advantages in storing the model grids in grib for both the modeler and the user. The advantages are 
 * compactness, and a standard format. The user also has the advantage of eliminating an extra interpolation 
 * step. The staggering information is stored in the last 4 bits of flag table 3.4. If these bits are all 
 * zero, there is no staggering. 
 * 
 ...
 *
 * This scheme allows encoding the Arakawa A-E Egrids. The wgrib2 -grid will show the staggering. The storage 
 * description was updated in v2.0.8 to be 
 * 
...
/**
 * Print staggered grid information.
 *
 * @param scan The scan information.
 * @param inv_out The output buffer.
 *
 * @author Wesley Ebisuzaki @date 1/2007
 */
static void print_stagger(int scan, char *inv_out) {
