/** @file
 * @brief Contains wgrib2 routine.
 * 
 * Originally the main of the wgrib2 utility. Later became main or routine depending on 
 * CALLABLE_WGRIB2. Finally, wgrib2 utility became a wrapper that calls the wgrib2 routine.
 * 
 * ### Program History Log
 ...
 * 2/2008 | W. Ebisuzaki | add -if support
 * 2/2008 | W. Ebisuzaki | fixed bug in processing of submessages
 * 2/2025 | W. Ebisuzaki | previously could be main() or wgrib2().  Now only wgrib2()
...
#include <setjmp.h>
jmp_buf fatal_err;      /**< Stores fatal error. */
...
int initial_call = 1;       /**< Flag indicating if this is the initial call. */
...
/* #define DEBUG */
#define CHECK               /**< Flag for validation checks. */

/* global variables .. can be modified by funtions */

int mode; 		            /**< Mode of operation. -2=finalize, -1=initialize,  0 .. N is verbosity mode  */
int header;		            /**< File header flag. */
int flush_mode;		        /**< Flush of output flag. (1 = yes) */
int WxText;		            /**< Decode NDFD keys. */

int save_translation;       /**< Flag to indicate whether to save translation information. */
...
int fix_ncep_2_flag;	    /**< Fix NCEP 2 flag. */
int fix_ncep_3_flag;	    /**< Fix NCEP 3 flag. */
int fix_ncep_4_flag;        /**< Fix NCEP 4 flag. */
int fix_undef_flag;         /**< Fix undefined flag. */

int for_mode;               /**< For loop mode flag. */
int for_start;              /**< For loop start value. */
int for_end;                /**< For loop end value. */
int for_step;               /**< For loop step value. */
int for_n_mode;             /**< For loop n mode flag. */
int for_n_start;            /**< For loop n start value. */
int for_n_end;              /**< For loop n end value. */
int for_n_step;             /**< For loop n step value. */

int match;                  /**< Run matching routines flag. */
int match_fs;               /**< Run fixed-string matching routines flag. */
extern int run_flag;        /**< Run flag. */

unsigned int last_message;	/**< Last message to process if set > 0. */

struct seq_file inv_file;               /**< Sequence file for inventory. */
struct seq_file rd_inventory_input;     /** Read inventory input file. */

enum input_type input;                          /**< Input type. */
enum output_order_type output_order;            /**< Output order type. */
enum output_order_type output_order_wanted;     /**< Desired output order type. */
enum geolocation_type geolocation;              /**< Geolocation type. */
...
int file_append;                /**< Append grib file flag. */
int dump_msg;                   /**< Dump message flag. */
int dump_submsg;                /**< Dump submessage flag. */
size_t dump_offset;             /**< Dump offset. */
int ieee_little_endian;         /**< IEEE little endian flag. */

int decode;		                /**< Decode grib file flag */

const char *item_deliminator;   /**< Item delimiter. */
const char *nl;                 /**< New line character string. */
const char *end_inv;	        /**< String to write after end of inventory. */

/* current grib location */
long int pos;                   /**< Current position in grib file. */
unsigned long int len;          /**< Length of current grib message. */
int submsg;                     /**< Current submessage number. */
int msg_no;                     /**< Current message number. */
int inv_no;                     /**< Current inventory number. */

double *lat = NULL;             /**< lat array - used by Latlon.c to store location information. */
double *lon = NULL;             /**< lat array - used by Latlon.c to store location information. */
int latlon;                     /**< Flag to indicate lat-lon grid processing. */
int warn_nonzero_min_sec;		/**< If reference time has nonzero minutes or seconds, warn if f_t(..) but not f_T(..) */
int GDS_change_no;              /**< GDS change number. */
int old_GDS_size;               /**< Old GDS size. */
int GDS_max_size = 0;           /**< Maximum GDS size. */
unsigned char *old_gds;         /**< Pointer to old GDS. */
int nx;                         /**< Number of grid points in x direction. */
int ny;                         /**< Number of grid points in y direction. */
int res;                        /**< Grid resolution. */
int scan;                       /**< Scan order mode. */
unsigned int nx_;               /**< Number of grid points in x direction (unsigned). */
unsigned int ny_;               /**< Number of grid points in y direction (unsigned). */
unsigned int npnts;             /**< Number of grid points. */
int use_scale;                  /**< Use scale flag. */
int input_scale;                /**< Input scale factor. */
int dec_scale;                  /**< Decimal scale factor. */
int bin_scale;                  /**< Binary scale factor. */
int max_bits;                   /**< Maximum number of bits. */
int wanted_bits;                /**< Desired number of bits. */
...
int version_if;		                /**< Version of IF block to use: 0-old stype 1-modern if */
...
/**
 * Set mode of operation (global variable).
 * 
 * Options for mode:
 * -2 = finalize
 * -1 = initialize
 * 0 to N = verbosity mode
 * 
 * @param new_mode New value of global mode variable.
 * 
 * @author Wesley Ebisuzaki @date 2005
 */
