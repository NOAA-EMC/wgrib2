/** @file
 * @brief Create GRIB2 GDT (Grid Definition Template) from GRIB2 message.
...
/** Grid Definition Template (GDT) definition. */
struct gdt_defn {
    int template_num;
    char *seq;
};
...
/** Table for the various grid templates to get from *sec[3]. */
...
/**
 * This routine takes the grib section and makes a g2lib igdtmpl(*) array for ipolates2 library.
 * 
 * igdt is used by NCEP grib and ipolates2 libraries.
 *
 * @param sec Pointer to the GRIB2 message sections. Only sec[3][*] is used.
 * @param igdtnum Pointer to output grid definition template number.
 * @param igdttmpl Pointer to output grid definition template array.
 * @param igdtleni Pointer to output grid definition template length.
 *
